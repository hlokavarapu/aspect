...
#include <aspect/postprocess/volume_flux_statistics.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    VolumeFluxStatistics<dim>::execute (TableHandler &statistics)
    {
      ...
      // volume flux and that is owned by this processor,
      // integrate the normal volume flux given by the formula
      //   j =  v * n
      //
      ...
      for (std::map<types::boundary_id, double>::const_iterator
        ...
        {
          const std::string name = "Outward volume flux through boundary with indicator "
      ...
      return std::pair<std::string, std::string> ("Volume fluxes through boundary parts:",
                                                  screen_text.str());
    }
  }
}
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(VolumeFluxStatistics,
                                  "volume flux statistics",
                                  "A postprocessor that computes some statistics about "
                                  "the volume flux across boundaries. For each boundary "
                                  "indicator (see your geometry description for which boundary "
                                  "indicators are used), the volume flux is computed in outward "
                                  "direction, i.e., from the domain to the outside, using the "
                                  "formula $\\int_{\\Gamma_i} \\mathbf v \\cdot \\mathbf n$ "
                                  "where $\\Gamma_i$ is the part of the boundary with indicator $i$, "
                                  "$\\mathbf v$ is the velocity, and $\\mathbf n$ is the outward normal. "
                                  "\n\n"
                                  "As stated, this postprocessor computes the \\textit{outbound} volume "
                                  "flux. If you "
