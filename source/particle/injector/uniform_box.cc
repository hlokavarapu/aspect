...
#include <aspect/particle/injector/uniform_box.h>
...
namespace aspect
{
  namespace Particle
  {
    namespace Injector
    {
      template <int dim>
      void
      UniformBox<dim>::inject_particles(std::multimap<types::LevelInd, Particle<dim> > &particles, types::particle_index index_offset)
      {
        ...
        types::particle_index particle_index = index_offset; 
                ...
                if (dim == 2)
                  {
                    ...
                    try
                      {
                        particles.insert(this->inject_particle(particle_position,particle_index));
                      }
      ...
      UniformBox<dim>::declare_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          ...
          {
            prm.declare_entry ("Number of tracers to inject", "100",
                               Patterns::Double (0),
                               "Total number of tracers to create periodically (not per processor or per element). "
      ...
      UniformBox<dim>::parse_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Tracers");
          {
            n_tracers    = static_cast<types::particle_index>(prm.get_double ("Number of tracers to inject"));
...
namespace aspect
{
  namespace Particle
  {
    namespace Injector
    {
      ASPECT_REGISTER_PARTICLE_INJECTOR(UniformBox,
                                         "uniform box",
                                         "Generate a uniform distribution of particles periodically"
